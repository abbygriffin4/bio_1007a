...
toupper(my_letters) # capitalize these letters
print(my_letters)
```
   + for the first variable, use a single line of code in R to select 4 random rows and replace the numerical values in those rows with NA.
```{r}
q3df<-data.frame(my_unifs, my_letters, stringsAsFactors = FALSE) # create a data frame of these two vectors
print(q3df)
...
```{r}
abc_q3df<-q3df[order(q3df$my_letters), ] # now alphabetize this vector
print(abc_q3df)
```
   + calculate the column mean for the first variable.
```{r}
mean(abc_q3df$my_unifs, na.rm=TRUE) # what is the mean of the first variable?
```
   + rename the second column ‘my_LETTERS’
```{r}
colnames(abc_q3df)[colnames(abc_q3df) == "my_letters"] ="my_LETTERS" # rename the column
print(abc_q3df)
```
